...
public interface ITagResolutionService
{
    ...
    Task EnsureUncategorizedAsync(Guid tagId);
}
...
public class TagResolutionService : ITagResolutionService
{
        ...
        foreach (var item in list)
        {
            ...
            else
            {
                ...
                if (entity.Id.HasValue)
                {
                    ...
                    await EnsureUncategorizedAsync(entity.Id.Value);
    ...
    public async Task EnsureUncategorizedAsync(Guid tagId)
    {
        ...
        var exists = await _db.TagTypes.AnyAsync(tt => tt.TagId == tagId && tt.TypeId == type.Id);
        if (!exists)
        {
            _db.TagTypes.Add(new TagTypes { TagId = tagId, TypeId = type.Id });
            await _db.SaveChangesAsync();
        }
    }
}
