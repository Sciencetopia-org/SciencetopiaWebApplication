...
- GET `api/StudyPlans/{id}`
  ...
  - 权限：需对该计划可读
  - 响应：
    - 基础：`{ id, title, description, currentVersionId, currentVersionNumber }`
    - 课节列表：按模块返回 `Prerequisite[] / MainCurriculum[] / AdvancedTopics[]`，每项：
      - `id, name, description, associatedKnowledgeNodes?`
      - `resources`: 为懒加载，值为 null（不包含资源清单与学习状态）
  - 说明：该接口仅提供课节框架，资源与学习状态请调用 Lesson 明细或 Progress 接口。
...
- GET `api/StudyPlans/{planId}/Lessons/{lessonId}`
  - 功能：懒加载获取单个 lesson 的详细信息（资源列表与是否已学）
  - 权限：需对该计划可读
